...
public class EntityCentricKnowledgeBase extends AbstractKnowledgeBase {
	...
	protected static final String VERTEXDEGREEFIELD = "WikidataVertexDegree";
	protected static final String KBMAINLINK = "Mainlink";

	/**
	 * This hashmap stores the Prior values of the standard Wikidata. Key: Lucene intern document id Value: Amount of
	 * occurrences of this entity
	 */
	protected Map<String, Integer> indexpriorHashMap;

	/**
	 * This hashmap stores the Sense Prior values of the standard Wikidata.
	 * <p>
	...
	public void initialize() {
		...
		try {
			...
			for (int i = 0; i < maxDoc; i++) {
				...
				final String vertexDegreeString = iReader.document(i).get(VERTEXDEGREEFIELD);
				String entity = iReader.document(i).get("Mainlink").replaceAll("http://www.wikidata.org/entity/", "");
				if ((val != null) && !val.equalsIgnoreCase("")) {
					...
					for (final String element : splitter) {
						...
						int vertexDegree = 0;
						try {
							occurences = Integer.valueOf(value[1]);
							vertexDegree = Integer.valueOf(vertexDegreeString);
						} catch (final NumberFormatException e) {
							logger.warn(this.kbName()+ " Warning NumberFormatException while Initialization:" + val + ". Taking default value 1 as prior.");
						}
						hash.put(value[0].toLowerCase(Locale.US).hashCode(), occurences + vertexDegree);
					}
	...
	class ECExternFeatures implements IEntityCentricExtFeatures {
		...
		public int getHashedPrior(String sf, String uri) {
			String entity = uri.replaceAll("http://www.wikidata.org/entity/", "");
			int res = 0;
