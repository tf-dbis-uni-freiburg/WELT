...
import welt.lucene.query.FuzzyLabelSimilarity;
	...
	public void processAlgorithm() {
		...
		try {
			...
			/*for(int i = 0; i < score.length; i++) {
				System.out.println(top.scoreDocs[i].doc + " - " + score[i]);
			}
			System.out.println();*/
...
//			}
	...
	private Query createQuery(EntityDisambiguationDPO dpo,
		...
		features.add(query.add(LuceneFeatures.queryStringFuzzy(
				dpo.getSelectedText(), "Label", fuzzyLabelSim, Occur.SHOULD,
				DisambiguationMainService.MAXCLAUSECOUNT), "Feature1", true));
		// Feature 2
		features.add(query.add(LuceneFeatures.queryStringTerm(
				dpo.getSelectedText(), "UniqueLabel", defaultSim,
				Occur.MUST, DisambiguationMainService.MAXCLAUSECOUNT), "Feature2",
				false));
		// Feature 3
		...
		// Feature 4
		features.add(query.add(LuceneFeatures.queryStringTerm(dpo.getContext(),
				"UniqueLabel", defaultSim, Occur.SHOULD,
		...
		features.get(1).setWeight(0.06771f);
		 ...
		 * for(int i = 0; i < 9; i++) {
			System.out.println(features.get(i).getName() + " - " + features.get(i).getWeight());
		}*/
