...
import welt.entitydisambiguation.algorithms.collective.AbstractWord2VecPageRank;
import welt.entitydisambiguation.algorithms.collective.Edge;
import welt.entitydisambiguation.algorithms.collective.Vertex;
...
class FinalEntityDisambiguation extends AbstractWord2VecPageRank {
	...
	public FinalEntityDisambiguation(EntityCentricKBWikidata eckb,
	...
	protected void buildMainGraph() {
		...
		for (SurfaceForm rep : repList) {
			...
			for (String s : arrList) {
				int prior = this.eckb.getFeatureDefinition().getHashedPrior(
...
//						}
	...
	public boolean analyzeResults(PageRankWithPriors<Vertex, Edge> pr) {
		...
		for (int i = 0; i < repList.size(); i++) {
			System.out.println("Surface form:" + repList.get(i).getSurfaceForm());
			for (Vertex v : vertexCol) {
				if (v.getEntityQuery() == repList.get(i).getQueryNr()) {
					System.out.println("Score for: "+v.getUris().get(0)+"  :  "+Math.abs(pr.getVertexScore(v)));
				}
			}
...
//					if (secondMax < threshold) {
						...
						System.out.println("I disambiguate: "+tempSolution);
