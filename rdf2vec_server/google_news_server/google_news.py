...
class Word2VecRest(Flask):
    ...
    def compute_w2vsimilarity(
        ...
        if domain == 'Wikidata':
            try:
                similarity = \
                    GunicornApplication.w2vmodel_dbpedia.similarity(s1,
                        s2)
            except Exception:
                similarity = 0

    def compute_w2vsimilarity_multi(
        self,
        l1,
        str,
        domain,
        ...
        if domain == 'Wikidata':
            for word in l1:
                try:
                    testword = \
                        GunicornApplication.w2vmodel_dbpedia.__getitem__(word)
        ...
        if len(l_conform):
            try:
                similarity = \
                    GunicornApplication.w2vmodel_dbpedia.n_similarity(l_conform,
                        l2)
            except Exception:
                similarity = 0
    ...
    def infer_docvector(self, s1, domain):
        if domain == 'wiki_german':
            return GunicornApplication.d2vmodel_german.infer_vector(s1,
                    steps=25)
        else:
            if GunicornApplication.d2vmodel is None:
                return 0
            else:
                return GunicornApplication.d2vmodel.infer_vector(s1,
                        steps=25)

    def compute_d2vsimilarity(
        self,
        d1,
        vec2,
        domain,
        ...
        if domain == 'wiki_german':
            try:
                ...
                if len(vec1) \
                    != GunicornApplication.d2vmodel_german.layer1_size:
                    similarity = 0
                else:
                    similarity = dot(matutils.unitvec(vec1),
                            matutils.unitvec(vec2))
                    if similarity > 0:
...
#            else :
         ...
         # Check whether entity is in model
                ...
                if len(vec1) \
                    != GunicornApplication.d2vmodel.layer1_size:
                    similarity = 0
                else:
                    similarity = dot(matutils.unitvec(vec1),
                            matutils.unitvec(vec2))
...
@w2v.route('/w2vsim', methods=['GET','POST'])
def w2vsim():
    ...
    for q in data:
        ...
        if len(split) > 2:
            ...
            for a in range(0, len(split) - 1):
                l.append(split[a])
            sim = w2v.compute_w2vsimilarity_multi(l, split[len(split)
                    - 1], domain)
        else:
...
def d2vsim():
    ...
    for i in range(0, len(candidates)):
        cansim[i] /= 10
        result = {'qryNr': sf['qryNr'], 'surfaceForm': sf['surfaceForm'
                  ], 'sim': cansim}
    f.append(result)
...
class GunicornApplication(BaseApplication):
    ...
    w2vmodel_dbpedia = KeyedVectors.load_word2vec_format('../models/GoogleNews-vectors-negative300.bin', binary=True)
    ...
    def __init__(self, wsgi_app, port=8801):
        self.options = {
    ...
    def load_config(self):
        config = dict([(key, value) for (key, value) in
                      self.options.iteritems() if key
